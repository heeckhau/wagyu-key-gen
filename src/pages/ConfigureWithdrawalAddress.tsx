...
const ConfigureWithdrawalAddress = () => {
  ...
  const validateInputs = async () => {
    ...
    if (inputWithdrawalAddress !== "") {
      const isValidAddress = await web3Utils.isAddress(inputWithdrawalAddress);
      if (!isValidAddress) {
    ...
    if (!isError) {
      ...
      eth2Deposit.validateBLSCredentials(network, mnemonic, inputIndex, inputCredentials).then(() => {
